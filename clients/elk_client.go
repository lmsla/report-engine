...
func TestElasticsearch(instance models.Instance) (error, models.Response) {
	...
	if err != nil {
		...
		res_t.Msg = "Error creating Elasticsearch client"
		return err, res_t
	}
	...
	if err != nil {
		...
		res_t.Msg = "Error ES getting cluster health"
		return err, res_t
	}
	defer res.Body.Close()

	if res.IsError() {
		res_t.Msg = "Elasticsearch response error"
		// res_t.Body = res.Body
		log.Logrecord("ERROR", fmt.Sprintf("Elasticsearch response error: %s", res.String()))
	} else {
		fmt.Println("Cluster Health Response:")
		fmt.Println(res.String())
		res_t.Msg = "ES Cluster Health Check Succeed"
		res_t.Success = true
	}
	return err, res_t
}
...
func TestKibana(instance models.Instance) (error, models.Response) {
	...
	if err != nil {
		...
		res_t.Msg = "Error creating HTTP request"
		return err, res_t
	}
	...
	if err != nil {
		...
		res_t.Msg = "Error connecting to Kibana"
		return err, res_t
	}
	...
	if resp.StatusCode == http.StatusOK {
		...
		res_t.Msg = "Kibana response error"
	}
	return err, res_t
}
