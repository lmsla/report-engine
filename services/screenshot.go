...
func Test() {
	...
	schedule, err := GetReportByScheduleID(1)
	if err != nil {
		fmt.Println(err)
	}
	for _, ggg := range schedule {
		fmt.Println(ggg.ID)
	}
}
...
func ScreenshotbyReport(reportID int) {
	...
	for _, data := range element_data {
		Screenshot_element(data.Type, data.Instance.URL, data.UID, data.Instance.User, data.Instance.Password)
	}

}

func Screenshot_element(element_type string, url string, uid string, user string, password string) {
	...
	case "visualiztion":
		...
		file := fmt.Sprintf("%s/%s.png", global.EnvConfig.Files.ScreenshotFile, uid)
