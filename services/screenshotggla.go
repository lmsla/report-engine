...
func ScreenshotDocker1([]entities.Report ) {
	...
	data, err := GetInstanceInReportbyReportID(reportdata.ReportID)
	if err != nil {
		fmt.Println(err)
	}
	reportinfo, err := GetReportByReportID(reportdata.ReportID)
	if err != nil {
		fmt.Println(err)
	}
